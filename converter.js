...
function updateFlags() {
    var fromFlagCode = fromCurrency.selectedOptions[0].getAttribute("data-flag");
    var toFlagCode = toCurrency.selectedOptions[0].getAttribute("data-flag");
    if (fromFlagCode && toFlagCode) {
        fromFlag.src = "flags/".concat(fromFlagCode, ".png");
        toFlag.src = "flags/".concat(toFlagCode, ".png");
    }
}
...
convertBtn.addEventListener("click", function () {
    ...
    if (!isNaN(amount) && exchangeRates[from] && exchangeRates[to]) {
        var convertedAmount = (amount / exchangeRates[from]) * exchangeRates[to];
        resultDiv.innerText = "Converted Amount: ".concat(convertedAmount.toFixed(2), " ").concat(to);
